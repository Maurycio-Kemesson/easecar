import 'package:flutter/material.dart';

class AppColors {
  static const Color primaryColor = Color(0xff0455A8);
  static const Color red = Colors.red;
  static const Color snackBarCustomSuccess = Colors.green;
  static const Color secondaryColor = Color(0xff043A58);
  static const Color white = Color(0xffFFFFFF);
  static const Color tertiaryColor = Color(0xff2D879B);
  static const Color textActivity = Color(0xff736464);
  static const Color green = Color(0xff1B9818);
  static const Color statePadding = Color(0xffDBCB38);
  static const Color lightGray = Color(0xffEDEDED);
  static const Color softGray = Color(0xffCAC4C4);
  static const Color softBlue = Color(0xffE6EEF6);
  static const Color darkerSoftBlue = Color(0xffC1D4E7);
  static const Color mediumGray = Color(0xff9D9D9D);
  static const Color brightBlue = Color(0xff1582F1);
  static const Color brightGreen = Color(0xff1B9818);
  static const Color intenseRedGreen = Color(0xffDA4430);
  static const Color softBlush = Color(0xffFDF5F5);
  static const Color steelBlue = Color(0xff2E3E4B);
  static const Color slateGray = Color(0xff737E86);
  static const Color deepNavyBlue = Color(0xff174574);
  static const Color lightGold = Color(0xffFADA68);
}
